...
McNetworkMenu is a native SwiftUI menu-bar utility for macOS. Its icon follows the interface carrying the default route: Wi-Fi uses the native `wifi` symbol, Ethernet uses the native `bonjour` symbol shown by macOS Network Settings, and offline uses `network.slash`.
